---
name: model-contract
description: Source-derived GLB, stable metadata and registered educational overlays
metadata:
  type: project
---
...
**How to apply:** One mesh per catalogue ID, matching extras and provenance. Membership is the original required-parts registry plus supplements, not a fixed-count assumption. Numeric coordinates are millimetres, +Y proximal, +Z anterior, +X medial. Clinical symbols in landmarks.json are preserved source landmarks, not clinically validated. Keep anatomical fascia separate from the frozen illustrative hulls and partial deltoid coverage explicit. Public repository and GitHub Pages hosting were authorized and completed. The canonical hosting path is .github/workflows/pages.yml, deploying dist to https://holsteredsoul.github.io/crus-atlas/ after tests and a build with base /crus-atlas/. Local development retains its relative base. Do not convert to a server stack.
...
The 2026-09-07 supplement review accepted source partitions/meshes through A–F;
G reconstructions remain excluded with specific registration gaps in
