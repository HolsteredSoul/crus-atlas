...
Required every session through AGENTS.md; some harness adapters also import it directly.
If it is not already in context, read it explicitly. One line per memory, hook only — the
file holds the content. Project rulings live here, versioned with the code; harness-native
