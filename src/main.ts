...
import { catalogue, byId, compartmentNames, typeNames, palette, type PartType } from './catalogue';
...
app.innerHTML=`
 ...
 <main class="workspace">
  <aside class="left-panel" aria-label="Anatomy layers">
   ...
   <div class="layer-tools"><button data-action="xray" id="xray-button" aria-pressed="false">${icon('bone')} X-ray bones</button><button data-action="fade" id="fade-button" aria-pressed="false">${icon('layers-3')} Fade others</button></div>
   <div class="layer-footer"><span class="status-dot"></span><span id="part-count">47 structures</span><span class="unit">mm</span></div>
  </aside>
  <section class="center-panel" aria-label="Anatomy viewer">
   <div class="viewport" id="viewport">
    <div class="viewport-top"><div class="view-title"><span class="eyebrow">REGIONAL ANATOMY</span><h2>Crus & ankle</h2><span id="view-caption">Right lower limb · anterolateral view</span></div><span class="schematic-badge" id="model-badge"><span></span> Schematic model</span></div>
  ...
  <aside class="right-panel" aria-label="Structure and clinical details">
   ...
   <div class="disclaimer">${icon('info')}<p><strong>For education only.</strong> Not a medical device. Not for diagnosis or surgical planning. Geometry and landmarks are schematic.</p></div>
  </aside>
 </main>
 <div class="toast" id="toast" role="status" aria-live="polite" hidden></div>
 <dialog id="help-dialog"><div class="dialog-top"><span class="eyebrow">CRUS / FIELD GUIDE</span><button class="icon-button" data-action="close-help" aria-label="Close help">${icon('x')}</button></div><h2>Explore the lower leg.</h2><p>Orbit around the right leg, select a structure, then explore its attachments and clinical landmarks.</p><dl class="key-list"><div><dt><kbd>F</kbd></dt><dd>Focus selected structure</dd></div><div><dt><kbd>H</kbd></dt><dd>Hide selected structure</dd></div><div><dt><kbd>I</kbd></dt><dd>Isolate selected structure</dd></div><div><dt><kbd>R</kbd></dt><dd>Restore atlas and camera</dd></div><div><dt><kbd>Esc</kbd></dt><dd>Clear selection and overlay</dd></div></dl><h3>About this model</h3><p>Original procedural stand-in, with ${catalogue.length} individually named parts. It is not a segmented cadaver or a validated anatomical reconstruction. +Y is proximal, +Z anterior and +X medial. Coordinates use nominal millimetres.</p><p>Toe insertions are described in the catalogue; detailed toes and proximal femur are outside the model. Compartment envelopes are illustrative.</p><h3>Overlay colour key</h3><div class="overlay-key">${Object.entries(overlayColors).map(([k,v])=>`<span><i style="background:${v}"></i>${({partial:'Partial tear',stress:'Bone stress',fluid:'Fluid / bursa',pressure:'Compartment pressure',landmark:'Nerve landmark'} as Record<string,string>)[k]??k}</span>`).join('')}</div><p class="dialog-disclaimer">Educational use only. Not a medical device. No diagnosis, treatment recommendations or surgical planning.</p></dialog>
`;
drawIcons();
...
const collapsed=new Set<string>(['joint','cartilage','tendon','ligament','fascia','nerve','bursa','retinaculum']);
...
function renderTree(){
 const matches=catalogue.filter(p=>`${p.displayName} ${p.latinName??''} ${p.id} ${p.compartment} ${p.id==='atfl'?'ATFL':p.id==='cfl'?'CFL':p.id==='tp_tendon'?'PTTD':''}`.toLowerCase().includes(query));
 const types=[...new Set(catalogue.map(p=>p.type))];
 const row=(p:typeof catalogue[number])=>`<div class="part-row${selected===p.id?' selected':''}${viewer?.hidden.has(p.id)?' is-hidden':''}"><button class="part-select" data-action="select" data-id="${p.id}" aria-pressed="${selected===p.id}"><span class="part-dot" style="background:${palette[p.type]}"></span><span>${p.displayName}</span></button><button class="eye-button" data-action="visibility" data-id="${p.id}" aria-label="${viewer?.hidden.has(p.id)?'Show':'Hide'} ${p.displayName}">${icon(viewer?.hidden.has(p.id)?'eye-off':'eye')}</button></div>`;
 $('#tree').innerHTML=matches.length?types.map(type=>{
  const parts=matches.filter(p=>p.type===type);if(!parts.length)return '';
  const closed=!query&&collapsed.has(type);const all=catalogue.filter(p=>p.type===type);const shown=all.some(p=>!viewer?.hidden.has(p.id));
  ...
  return `<section class="tree-system"><div class="system-header"><button class="system-disclosure" data-action="system" data-id="${type}" aria-expanded="${!closed}">${icon(closed?'chevron-right':'chevron-down')}<span>${typeNames[type]}</span><span class="system-count">${parts.length}</span></button><button class="group-solo" data-action="solo-system" data-id="${type}" title="Solo ${typeNames[type]}">Solo</button><button class="eye-button" data-action="system-visibility" data-id="${type}" aria-label="Toggle ${typeNames[type]}">${icon(shown?'eye':'eye-off')}</button></div>${closed?'':`<div class="system-children">${children}</div>`}</section>`;
...
function updateButtons(){
 ...
 document.querySelectorAll<HTMLElement>('.condition-chip').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.id===activeCondition?.id)));$('#clear-condition').hidden=!activeCondition;
}
...
function activateCondition(id:string){
 const c=conditions.find(c=>c.id===id);if(!c)return;activeCondition=c;tab='clinical';selected=c.parts[0];viewer?.select(selected);viewer?.showCondition(c);viewer?.preset(c.view,false);viewer?.focus(selected);$('#view-caption').textContent=`Right lower limb · ${c.view.toLowerCase()} landmark view`;renderTree();renderDetail();updateButtons();
}
...
app.addEventListener('click',event=>{
 ...
 switch(action){
  ...
  case 'system-visibility':viewer?.toggle(catalogue.filter(p=>p.type===id).map(p=>p.id));renderTree();break;
  case 'solo-system':viewer?.isolate(catalogue.filter(p=>p.type===id).map(p=>p.id));renderTree();updateButtons();break;
  ...
  case 'locate':if(activeCondition){viewer?.preset(activeCondition.view,false);viewer?.focus(activeCondition.parts[0]);}break;
...
try{
 viewer=new AtlasViewer($('#viewport'));viewer.onSelect=selectPart;viewer.onStatus=toast;viewer.select(selected);
 viewer.onModel=()=>{selected=null;activeCondition=null;$('#model-badge').textContent='Imported GLB';setExplode(0);renderTree();renderDetail();updateButtons();};
 if(import.meta.env.VITE_ATLAS_GLB_URL)viewer.load(import.meta.env.VITE_ATLAS_GLB_URL);
