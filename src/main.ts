...
app.innerHTML=`
 ...
 <main class="workspace">
  <aside class="left-panel" aria-label="Anatomy layers">
   ...
   <details class="tissue-legend"><summary>Tissue colours</summary><div>${(['bone','muscle','tendon','ligament','fascia','cartilage','sheath','retinaculum'] as PartType[]).map(t=>`<span><i style="background:${palette[t]}"></i>${t==='tendon'?'Tendons / aponeuroses':typeNames[t]}</span>`).join('')}</div><small>Educational colours · injury colours are shown in Clinical notes.</small></details><div class="layer-tools"><button data-action="xray" id="xray-button" aria-pressed="false">${icon('bone')} X-ray bones</button><button data-action="fade" id="fade-button" aria-pressed="false">${icon('layers-3')} Fade others</button></div>
...
const collapsed=new Set<string>(['joint','cartilage','tendon','ligament','fascia','nerve','bursa','retinaculum','sheath']);
